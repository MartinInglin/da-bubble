...
export class EditCurrentUserComponent implements OnInit, OnDestroy {
  ...
  showFileTypeError(): void {
    this.snackbarService.openSnackBar(
      'Please select a jpg or png file format.',
      'Close'
    );
  }
   ...
   * @returns {Promise<void>}
   */
  async changeMailAddress(): Promise<void> {
    if (this.currentUser && this.password) {
      ...
      if (isValid) {
        ...
        this.snackbarService.openSnackBar(`Verification email sent to ${this.updatedEmail}.`, 'Close');
        this.dialogRef.close();
      } else {
  ...
  async updatePassword(): Promise<void> {
    ...
    this.snackbarService.openSnackBar('Password changed successfully.', 'Close');
  }
  ...
  handleInvalidPassword(): void {
    this.passwordIsFalse = true;
    this.snackbarService.openSnackBar('The old password is incorrect.', 'Close');
  }
  ...
  /**
   * Checks if the updated name is empty.
   * @returns {boolean} True if the name is empty, otherwise false.
   */
  isNameEmpty(): boolean {
    return !this.updatedName || this.updatedName.trim().length === 0;
  }
}
